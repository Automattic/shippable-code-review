...
The finalized architecture for v1, derived from several grill sessions of earlier drafts (now consolidated into this doc; the drafts live in git history):

This is the single authoritative design doc; where any other doc disagrees, this one wins.

> Vendored onto `rebuild/incremental` from `rebuild/plan-on-main@a00d8d5` on
> 2026-07-15 so the tie-breaker travels with the migration work. One amendment
> since vendoring: §1.2 `ResponseIntent` gained `"respond"` (see
> `docs/plans/v1-incremental-migration.md`, intent-vocabulary decision).
> Execution strategy: `docs/plans/v1-incremental-migration.md` governs (§17's
> one-shot order is superseded).
...
type ResponseIntent = "accept" | "reject" | "respond"; // "respond" added 2026-07-14: the neutral reply (answers, follow-ups, "fixed — look again"). Verdict-forcing moved to resolution semantics: a blocker/finding thread stays unresolved until a verdict reply exists. See docs/plans/v1-incremental-migration.md, intent-vocabulary decision.

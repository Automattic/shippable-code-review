...
When you open a ChangeSet, the plan overlay tells you what the diff is doing and where to start reading. The headline is mechanical (verbatim from `ChangeSet.title`); the intent claims and the entry points are generated by an AI.

If an Anthropic credential is configured, the request fires automatically as soon as the ChangeSet is loaded — you don't have to click anything. A small "Auto-sending diff to Claude" chip in the plan header reminds you that the diff is leaving your machine, and a "regenerate" button next to it lets you re-run on demand (with a confirmation prompt, since tokens cost money). Without a key, the overlay still works: it falls back to today's rule-based plan (file/symbol map + structural intent) and offers a "configure api key to enable AI plan" link that opens the credentials panel.
...
─────────                              ───────────          ─────────────────────────────
cs loaded + hasAnthropicCredential
  └─ PlanProvider.trigger(cs)
     (in-session cache check, idempotent)
     │
     └─ POST /api/plan
        { changeset: ChangeSet }
                                ─────▶ buildStructureMap(cs)        (no model)
                                       ...
                                       ◀──── { plan: ReviewPlan }
overlay rebuilds
   AI claims + "Where to start"
   (rule-based map collapses behind
    an "all files" disclosure)
```

The frontend never talks to a model directly. The server is the only place that holds API keys.

`PlanProvider` (`web/src/PlanProvider.tsx`) sits above the workspace and caches one response per `(cs.id, content-hash)` pair. The cache is in-session only — switching to a different CS and back returns instantly, but reloading the app refetches. The same provider feeds both the plan overlay and the file sidebar's priority ranking, so the plan is reachable even when the overlay is closed.

Two kinds of requests can be in flight:

- **auto** — fired by `trigger(cs)` on first sight of a CS with a key configured. Never aborted on CS switch; the result lands in the cache for that CS so a revisit is instant.
- **regenerate** — fired by `regenerate(cs)` after the user confirms in the modal. Aborted if the user switches away while it is still in flight, with the previous cache entry restored.

If a request errors, the cache slot moves to status `fallback`. The overlay then renders today's full rule-based map with an error banner. The next time the user opens the plan overlay for that CS we silently retry (`regenerate(cs)` from the same effect that watches `showPlan`).
...
The server is a hard dependency now (`AGENTS.md` → "Deployment modes"), so `/api/health` failures never reach `PlanProvider` — they're caught by `ServerHealthGate` at boot. What `PlanProvider` *does* handle is errors after the auto-fire: API errors, model refusals, or `parsed_output: null`. Those land in `status: "fallback"` with the message preserved, and the plan overlay reverts to today's full rule-based map plus a "AI plan failed — showing rule-based fallback: …" banner. Opening the overlay again silently retries.

Without an API key, the auto-fire never runs. The overlay shows the rule-based plan (structure map, diagram view, rough intent) plus a "configure api key to enable AI plan" link that opens the credentials panel.

The privacy decision used to live behind an explicit "Send to Claude" click. With auto-fire it lives behind the key-configuration step instead: configuring a key means agreeing that diffs will be sent to Claude on every review. The persistent header chip is the standing reminder.
...
- `web/src/PlanProvider.tsx` — the in-session cache, the auto-fire + regenerate coordinators, and the abort logic for switch-during-regenerate.
- `web/src/usePlan.ts` — thin hook that binds the provider to a single `cs`. Returns `{ plan, questions, status, error, generate, regenerate }`.
...
- `web/src/components/ReviewPlanView.tsx` — the overlay. `aiEnabled` swaps the layout: hide the structure map while loading/ready, promote the AI entry points into a "Where to start" section, collapse the rest behind an "all files" disclosure. Without `aiEnabled` (no key, or gallery), it renders the original full-Map layout.

...
1. **`BlockOrigin` dirty `context: blob`** (`v1-architecture.md:28`) is not a real TS type. The more-grilled predecessor (`rebuild-plan.md` #15) specified a welded window of `DiffLine[]`. **Decision:** `context: DiffLine[]` (reuses the carried `DiffLine`). If you'd rather store a raw file snapshot string + offset, change it here before Task 1 — it's the one place it's defined.
